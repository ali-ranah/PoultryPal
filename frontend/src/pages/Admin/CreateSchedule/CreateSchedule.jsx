  ...
  if (!isAdmin) {
   ...
   const handleEmployeeSelect = (event) => {
    ...
    const handleWeekRecordChange = (index, field, value) => {
        ...
        if (field === 'date') {
            ...
            if (field === 'shiftStart') {
                ...
                if (updatedRecords[index].shiftEnd && updatedRecords[index].shiftEnd < updatedRecords[index].shiftStart) {
    ...
    const addNewDayRecord = () => {
        setWeekRecords([
            ...weekRecords,
            { date: '', shiftStart: '', shiftEnd: '', tasks: [{ description: '', duration: 0 }] }
        ]);
    };
    ...
    const removeTask = (dayIndex, taskIndex) => {
        const updatedRecords = [...weekRecords];
        updatedRecords[dayIndex].tasks = updatedRecords[dayIndex].tasks.filter((_, i) => i !== taskIndex);
        setWeekRecords(updatedRecords);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await AxiosRequest.post('/api/schedule/createSchedule', {
    ...
    return (
        <div className="flex items-center justify-center text-center min-h-screen bg-[#f0ebe4] font-poppins">
            ...
            >
                <Card className="p-8 space-y-4 shadow-lg rounded-lg bg-white">
                    <Typography variant="h4" className="mb-6 text-black">
                        Create Weekly Schedule
                    </Typography>
                    
                    <FormControl fullWidth sx={{ mb: 2 }}>
    <InputLabel id="employee-select-label" sx={{ color: 'black' }}>Select Employee</InputLabel>
    <Select
            ...
            <MenuItem key={employee._id} value={String(employee._id)}>
                {`${employee.firstName} ${employee.lastName}`} - {employee.email}
                    ...
                    {weekRecords.map((record, index) => (
                        <div key={index} className="p-4 border mb-6 rounded-lg bg-gray-50 shadow-inner">
                            <div className="flex justify-between items-center mb-4">
                                <Typography variant="h6" className="text-black">Day {index + 1}</Typography>
                                <Button
    ...
    <Input
        ...
        label="Date"
        color='black'
        value={record.date ? record.date.toISOString().slice(0, 10) : ''} // Format date to YYYY-MM-DD
...
<div className="flex space-x-4">
    <Input
        type="time"
        label="Shift Start"
        value={record.shiftStart ? record.shiftStart.toTimeString().slice(0, 5) : ''} // Format time to HH:MM
    ...
    <Input
        type="time"
        label="Shift End"
        value={record.shiftEnd ? record.shiftEnd.toTimeString().slice(0, 5) : ''} // Format time to HH:MM
        onChange={(e) => handleWeekRecordChange(index, 'shiftEnd', e.target.value)}
        className="focus:!ring-0"
        required
        min={record.shiftStart ? record.shiftStart.toTimeString().slice(0, 5) : ''} // Ensure min value is formatted correctly
    />
    ...
    <Input
        ...
        onChange={(e) => {
            ...
            if (hours === 1 && currentMinutes > 0) {
                ...
                handleTaskChange(index, taskIndex, 'duration', hours * 60 + currentMinutes);
            } else {
...
<div>
    <Input
        ...
        onChange={(e) => {
            ...
            // Update total duration
            handleTaskChange(index, taskIndex, 'duration', currentHours * 60 + minutes);
        }}
        className="focus:!ring-0"
        required
    />
                    ...
                    <Button onClick={addNewDayRecord} color="teal" className="mb-4 w-full">
                        Add New Day Record
                    </Button>
